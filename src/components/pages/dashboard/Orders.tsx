            ...
            {filteredOrders.map((order) => (
                <Box key={order._id} sx={{ display: 'flex', width: '80vw', justifyItems: 'center', alignItems: 'center', marginLeft: '2em' }}>
                    <Accordion sx={{ minHeight: '5em', border: '0.1em #004d40 solid', backgroundColor: '#b2dfdb' }}>
                        ...
                        >
                            <Box sx={{ display: 'flex', width: '70vw', justifyContent: 'space-around', alignItems: 'center'}}>
                                <Typography sx={{ width: '16%', flexShrink: 0 , textAlign: 'center', fontSize: '1.2rem'}}>{order.price}</Typography>
                                <Typography sx={{ width: '16%', flexShrink: 0 , textAlign: 'center', fontSize: '1.2rem'}}>{new Date(order.orderTime).toLocaleDateString('en-GB')}</Typography>
                                <Typography sx={{ width: '16%', flexShrink: 0 , textAlign: 'center', fontSize: '1.2rem'}}>{order.shippingDetails.orderType}</Typography>
                                <Typography sx={{ width: '16%', flexShrink: 0 , textAlign: 'center', fontSize: '1.2rem'}}>{order.status}</Typography>
                            </Box>
                            <Box sx={{ display: 'flex', width: '20vw', justifyContent: 'center', alignItems: 'center' }}>
                                <Box>
                                    <Button variant="outlined" disabled={order.status !== 'processing'} onClick={handleChangeStatus(order, 'cancelled')} sx={{ color: 'black', border: 'none', marginLeft:'-0.5em', backgroundColor:'#26a69a','&:hover':{border:'none',backgroundColor:'#4db6ac'}}} startIcon={<Clear sx={{ color: '#004d40'}} />}>
                                        cancel
                                    </Button>
                                </Box>
                                <Box>
                                    <Button variant="outlined" disabled={order.shippingDetails.orderType !== 'pickup' || order.status !== 'processing'} onClick={handleChangeStatus(order, 'accepted')} sx={{ color: 'black', border: 'none', marginLeft:'0.5em', backgroundColor:'#26a69a','&:hover':{border:'none',backgroundColor:'#4db6ac'} }} startIcon={<ChangeCircle sx={{ color: '#004d40' }} />}>
                                        complete 
                                    </Button>
                                </Box>
                            </Box>
